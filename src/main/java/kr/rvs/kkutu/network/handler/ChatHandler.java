...
import kr.rvs.kkutu.network.packet.impl.ChatInPacket;
import kr.rvs.kkutu.network.packet.impl.ChatOutPacket;
import kr.rvs.kkutu.network.packet.impl.YellPacket;
...
public class ChatHandler implements PacketHandler, EventHandler<KeyEvent> {
    ...
    public void handle(KeyEvent event) {
        ...
        if (event.getCode() == KeyCode.ENTER) {
            PacketManager.get().sendPacket(new ChatOutPacket(textField.getText()));
            textField.setText("");
        }
    }
}
